...
/**
 * ProjectAdditionForm - Submit new research project proposal
 * Uses custom hooks for submission and state management
 * All API calls handled through rspcApi service layer
 */
function ProjectAdditionForm({ setActiveTab }) {
  ...
  const [loadingProfIDs, setLoadingProfIDs] = useState(false);

  // Fetch professor IDs on mount
  useEffect(() => {
    const loadProfIDs = async () => {
      ...
      setLoadingProfIDs(true);
      try {
        const data = await fetchProfessorIDs();
        if (Array.isArray(data) && data.length > 0) {
          localStorage.setItem("profIDs", JSON.stringify(data));
          setProfIDs(data);
        }
      } catch (error) {
        console.error("Error fetching professor IDs:", error);
      } finally {
        setLoadingProfIDs(false);
      }
    };
  ...
  const form = useForm({
    initialValues: {
      ...
      description: "",
      duration: 0,
      submission_date: getISODateString(),
      budget: [],
      overhead: "",
      sanction_date: "",
      sanctioned_amount: "",
  ...
  // Create handler for form submission with API call
  const submitFormData = async (values) => {
    ...
    await submitProjectAddition(formData);
